...
def calculate_thermalSA(stage_4K, stage_800mK, stage_100mk, stage_10mK, freq):
    ...
    for i, temp in enumerate(stage):
        ax.plot(f, stage_noise[i],  color = color_list[i], label='{temp}K')
    ...
    # for i, temp in enumerate(stage):
    #     stage_noise_att[i,:] = JN_noise(temp, freq)*att_list[i]

    # for i, temp in enumerate(stage):
    #     if stage_check["800mK"] != 0:
    #         ax.plot(f, stage_noise_att[i],  color = color_list[i], label='{temp}K')
    #     else:
    #         pass
