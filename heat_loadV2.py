...
def calculate_thermalSA(stage_50K, stage_4K, stage_800mK, stage_100mk, stage_20mK, freq, interest_freq):
    ...
    for i, temp in enumerate(stage):
        ...
        ax.plot(freq, stage_noise_att[i],  color = color_list[i], label=f'{temp}K att', linestyle=(0, (3, 5, 1, 5, 1, 5)))
    total_temperature = sum(stage_noise_att)+stage_noise[-1]
    ax.plot(freq, total_temperature, label='effect temperature', lw=3, color='r')


    # ax.plot(freq, JN_noise(inversJN(total_temperature[np.where(freq==6e9)], freq), freq), ls='-.', label='jn')
    efft_temp = inversJN(total_temperature[np.where(freq==interest_freq)], interest_freq)[0]


    ax.plot(freq, JN_noise(efft_temp, freq), ls='-.', lw=3)
    ax.fill_between(freq, JN_noise(efft_temp-0.02, freq), JN_noise(efft_temp+0.02, freq), alpha=0.3)
    ax.axvline(interest_freq, linestyle="--")
    plt.title(f'effect temperature at {interest_freq/1e9:.2f} GHz = {efft_temp:.3f}K')
    ax.legend()
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.set_ylabel('Power')
...
if __name__=="__main__":
    f = np.arange(1e7, 1e13, 1e7)
    calculate_thermalSA(0,23,9,0,26,f, 5e9)  #4K, 800mK, 100mK, 10mK
